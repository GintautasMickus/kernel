...
#define SC2363_DRIVER_NAME "sc2363"

#define SC2363_AEC_PK_LONG_GAIN_HIGH_REG 0x3e08
#define SC2363_AEC_PK_LONG_GAIN_LOW_REG 0x3e09

#define SC2363_AEC_PK_LONG_EXPO_HIGH_REG 0x3e01
#define SC2363_AEC_PK_LONG_EXPO_LOW_REG 0x3e02

#define SC2363_AEC_GROUP_UPDATE_ADDRESS 0x3812
#define SC2363_AEC_GROUP_UPDATE_START_DATA 0x00
#define SC2363_AEC_GROUP_UPDATE_END_DATA 0x30

#define SC2363_PIDH_ADDR 0x3107
#define SC2363_PIDL_ADDR 0x3108

/* High byte of product ID */
#define SC2363_PIDH_MAGIC 0x22
/* Low byte of product ID  */
#define SC2363_PIDL_MAGIC 0x32

#define SC2363_EXT_CLK 24000000
#define SC2363_TIMING_VTS_HIGH_REG 0x320e
#define SC2363_TIMING_VTS_LOW_REG 0x320f
#define SC2363_TIMING_HTS_HIGH_REG 0x320c
#define SC2363_TIMING_HTS_LOW_REG 0x320d
#define SC2363_FINE_INTG_TIME_MIN 0
#define SC2363_FINE_INTG_TIME_MAX_MARGIN 0
#define SC2363_COARSE_INTG_TIME_MIN 1
#define SC2363_COARSE_INTG_TIME_MAX_MARGIN 4
#define SC2363_HORIZONTAL_OUTPUT_SIZE_HIGH_REG 0x3208
#define SC2363_HORIZONTAL_OUTPUT_SIZE_LOW_REG 0x3209
#define SC2363_VERTICAL_OUTPUT_SIZE_HIGH_REG 0x320a
#define SC2363_VERTICAL_OUTPUT_SIZE_LOW_REG 0x320b

#define SC2363_MIRROR_FILP 0x3221

static struct sc_camera_module sc2363;

/* ======================================================================== */
/* Base sensor configs */
/* ======================================================================== */
/* MCLK:24MHz  1920x1080  30fps   mipi 2lane  10bit  390Mbps/lane */
static struct sc_camera_module_reg sc2363_init_tab_1920_1080_30fps[] = {
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x0103, 0x01},
{SC_CAMERA_MODULE_REG_TYPE_TIMEOUT, 0x0000, 0x10},
...
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3802, 0x00},
...
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3208, 0x07},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3209, 0x88},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x320a, 0x04},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x320b, 0x40},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3200, 0x00},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3201, 0x00},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3202, 0x00},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3203, 0x00},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3204, 0x07},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3205, 0x8f},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3206, 0x04},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3207, 0x47},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3211, 0x04},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3213, 0x04},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x4800, 0x64},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x5988, 0x02},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x598e, 0x07},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x598f, 0xd5},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x4837, 0x33},
...
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3e01, 0x4d},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3e02, 0xe0},
...
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3e08, 0x03},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3e09, 0x10},
{SC_CAMERA_MODULE_REG_TYPE_DATA, 0x3221, 0x00}
};

/* ======================================================================== */

static struct sc_camera_module_config sc2363_configs[] = {
	{
		...
		.reg_table = (void *)sc2363_init_tab_1920_1080_30fps,
		.reg_table_num_entries = ARRAY_SIZE(sc2363_init_tab_1920_1080_30fps),
		.v_blanking_time_us = 3078,
		PLTFRM_CAM_ITF_MIPI_CFG(0, 2, 390, SC2363_EXT_CLK)
	}
};

static int sc2363_set_flip(
	...
	int len)
{
	...
	if (!IS_ERR_OR_NULL(cam_mod->active_config)) {
		...
		if (PLTFRM_CAMERA_MODULE_IS_FLIP(mode))
			orientation |= 0x60;
                for (i = 0; i < len; i++) {
                        if (reglist[i].reg == SC2363_MIRROR_FILP)
...
static int SC2363_g_VTS(struct sc_camera_module *cam_mod, u32 *vts)
{
	u32 msb, lsb;
	int ret;
...
static int sc2363_auto_adjust_fps(struct sc_camera_module *cam_mod,
	u32 exp_time)
{
	int ret;
	u32 vts;
	...
	if ((exp_time + SC2363_COARSE_INTG_TIME_MAX_MARGIN) > 2 * cam_mod->vts_min)
		vts = (exp_time + SC2363_COARSE_INTG_TIME_MAX_MARGIN) >> 1;
	else
		vts = cam_mod->vts_min;
...
static int sc2363_set_vts(struct sc_camera_module *cam_mod,
	u32 vts)
{
	int ret = 0;
...
static int sc2363_write_aec(struct sc_camera_module *cam_mod)
{
	int ret = 0;
	...
	if ((cam_mod->state == SC_CAMERA_MODULE_SW_STANDBY) ||
		...
		if (!IS_ERR_VALUE(ret) && cam_mod->auto_adjust_fps)
			ret |= sc2363_auto_adjust_fps(cam_mod,
				cam_mod->exp_config.exp_time);
		...
		/* hold reg start */
		ret = sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_GROUP_UPDATE_ADDRESS,
			SC2363_AEC_GROUP_UPDATE_START_DATA);

		ret |= sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_PK_LONG_GAIN_HIGH_REG,
			(a_gain >> 8) & 0xff);
		ret |= sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_PK_LONG_GAIN_LOW_REG,
			a_gain & 0xff);

		if (a_gain < 0x20) {//1x~2x
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0x06);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x48);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x08);
		} else if (a_gain < 0x40) {//2x~4x
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0x14);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x48);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x08);
		} else if (a_gain < 0x80) {//4x~8x
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0x18);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x48);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x08);
		} else if (a_gain < 0xf8) {//8x~15.5x
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0x13);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x48);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x08);
		} else if (a_gain < 0x1f0) {//15.5x~31x
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0xa1);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x78);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x48);
		} else {//31x~
			ret |= sc_camera_module_write_reg(cam_mod, 0x3301, 0xa1);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3306, 0x78);
			ret |= sc_camera_module_write_reg(cam_mod, 0x3632, 0x78);
		}

		ret |= sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_PK_LONG_EXPO_HIGH_REG,
			(exp_time >> 4) & 0xff);
		ret |= sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_PK_LONG_EXPO_LOW_REG,
			(exp_time & 0x0f) << 4);

		if (((exp_time >> 4) & 0xff) < 0x05)
			ret |= sc_camera_module_write_reg(cam_mod, 0x3314, 0x12);
		else if (((exp_time >> 4) & 0xff) > 0x0a)
			ret |= sc_camera_module_write_reg(cam_mod, 0x3314, 0x02);

		if (!cam_mod->auto_adjust_fps)
			ret |= sc2363_set_vts(cam_mod, cam_mod->exp_config.vts_value);

		/* hold reg end */
		ret |= sc_camera_module_write_reg(cam_mod,
			SC2363_AEC_GROUP_UPDATE_ADDRESS,
			SC2363_AEC_GROUP_UPDATE_END_DATA);
...
static int sc2363_g_ctrl(struct sc_camera_module *cam_mod, u32 ctrl_id)
{
	int ret = 0;
...
static int sc2363_filltimings(struct sc_camera_module_custom_config *custom)
{
	int i, j;
	...
	for (i = 0; i < custom->num_configs; i++) {
		...
		for (j = 0; j < reg_table_num_entries; j++) {
			...
			case SC2363_TIMING_VTS_HIGH_REG:
				...
				break;
			case SC2363_TIMING_VTS_LOW_REG:
				timings->frame_length_lines =
					(reg_table[j].val |
					(timings->frame_length_lines & 0xff00));
				break;
			case SC2363_TIMING_HTS_HIGH_REG:
				...
				timings->line_length_pck =
					...
					timings->line_length_pck);
				break;
			case SC2363_TIMING_HTS_LOW_REG:
				timings->line_length_pck =
					(reg_table[j].val |
					(timings->line_length_pck & 0xff00));
				break;
			case SC2363_HORIZONTAL_OUTPUT_SIZE_HIGH_REG:
				timings->sensor_output_width =
					((reg_table[j].val << 8) |
					(timings->sensor_output_width & 0xff));
				break;
			case SC2363_HORIZONTAL_OUTPUT_SIZE_LOW_REG:
				timings->sensor_output_width =
					(reg_table[j].val |
					(timings->sensor_output_width & 0xff00));
				break;
			case SC2363_VERTICAL_OUTPUT_SIZE_HIGH_REG:
				timings->sensor_output_height =
					((reg_table[j].val << 8) |
					(timings->sensor_output_height & 0xff));
				break;
			case SC2363_VERTICAL_OUTPUT_SIZE_LOW_REG:
				timings->sensor_output_height =
					(reg_table[j].val |
...
static int sc2363_g_timings(struct sc_camera_module *cam_mod,
	struct sc_camera_module_timings *timings)
{
	int ret = 0;
	unsigned int vts;
...
static int sc2363_s_ctrl(struct sc_camera_module *cam_mod, u32 ctrl_id)
{
	int ret = 0;
...
static int sc2363_s_ext_ctrls(struct sc_camera_module *cam_mod,
...
static int sc2363_start_streaming(struct sc_camera_module *cam_mod)
{
	int ret = 0;

	sc_camera_module_pr_info(cam_mod, "active config=%s\n", cam_mod->active_config->name);

	ret = SC2363_g_VTS(cam_mod, &cam_mod->vts_min);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int sc2363_stop_streaming(struct sc_camera_module *cam_mod)
{
	int ret = 0;
...
static int sc2363_check_camera_id(struct sc_camera_module *cam_mod)
{
	u32 pidh, pidl;
	int ret = 0;
	...
	ret |= sc_camera_module_read_reg(cam_mod, 1, SC2363_PIDH_ADDR, &pidh);
	ret |= sc_camera_module_read_reg(cam_mod, 1, SC2363_PIDL_ADDR, &pidl);
	...
	if (pidh == SC2363_PIDH_MAGIC && pidl == SC2363_PIDL_MAGIC) {
		sc_camera_module_pr_info(cam_mod,
		...
		sc_camera_module_pr_err(cam_mod,
			...
			SC2363_PIDH_MAGIC, SC2363_PIDL_MAGIC, pidh, pidl);
		ret = -EINVAL;
		goto err;
	}
...
static struct v4l2_subdev_core_ops sc2363_camera_module_core_ops = {
...
static struct v4l2_subdev_video_ops sc2363_camera_module_video_ops = {
	.enum_frameintervals = sc_camera_module_enum_frameintervals,
	.s_mbus_fmt = sc_camera_module_s_fmt,
	.g_mbus_fmt = sc_camera_module_g_fmt,
	.try_mbus_fmt = sc_camera_module_try_fmt,
...
static struct v4l2_subdev_ops sc2363_camera_module_ops = {
	.core = &sc2363_camera_module_core_ops,
	.video = &sc2363_camera_module_video_ops
};

static struct sc_camera_module_custom_config sc2363_custom_config = {
	.start_streaming = sc2363_start_streaming,
	.stop_streaming = sc2363_stop_streaming,
	.s_ctrl = sc2363_s_ctrl,
	.g_ctrl = sc2363_g_ctrl,
	.s_ext_ctrls = sc2363_s_ext_ctrls,
	.g_timings = sc2363_g_timings,
	.set_flip = sc2363_set_flip,
	.check_camera_id = sc2363_check_camera_id,
	.s_vts = sc2363_auto_adjust_fps,
	.configs = sc2363_configs,
	.num_configs = ARRAY_SIZE(sc2363_configs),
...
static int sc2363_probe(
	struct i2c_client *client,
	const struct i2c_device_id *id)
{
	dev_info(&client->dev, "probing...\n");

	sc2363_filltimings(&sc2363_custom_config);

	v4l2_i2c_subdev_init(&sc2363.sd, client,
		&sc2363_camera_module_ops);
	sc2363.sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;

	sc2363.custom = sc2363_custom_config;

	mutex_init(&sc2363.lock);
...
static const struct i2c_device_id sc2363_id[] = {
	{ SC2363_DRIVER_NAME, 0 },
	{ }
};

static struct of_device_id sc2363_of_match[] = {
	{.compatible = "smartsens,sc2363-v4l2-i2c-subdev"},
	{},
};

MODULE_DEVICE_TABLE(i2c, sc2363_id);

static struct i2c_driver sc2363_i2c_driver = {
	.driver = {
		.name = SC2363_DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = sc2363_of_match
	},
	.probe = sc2363_probe,
	.remove = sc2363_remove,
	.id_table = sc2363_id,
};

module_i2c_driver(sc2363_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc2363");
