...
static void sc_camera_module_reset(
...
static void sc_camera_module_set_active_config(
	struct sc_camera_module *cam_mod,
	struct sc_camera_module_config *new_config)
{
...
static int sc_camera_module_write_config(
	struct sc_camera_module *cam_mod)
{
	int ret = 0;
...
static int sc_camera_module_attach(
	struct sc_camera_module *cam_mod)
{
	int ret = 0;
	...
	struct v4l2_mbus_framefmt *fmt)
{
	...
	config = sc_camera_module_find_config(cam_mod, fmt, NULL);
	...
	if (cam_mod->frm_intrvl_valid &&
		!IS_ERR_OR_NULL(sc_camera_module_find_config(
...
int sc_camera_module_s_frame_interval(
	...
	struct v4l2_subdev_frame_interval *interval)
{
	...
	config = sc_camera_module_find_config(
			cam_mod,
...
int sc_camera_module_g_frame_interval(
	struct v4l2_subdev *sd,
	struct v4l2_subdev_frame_interval *interval)
{
	struct sc_camera_module *cam_mod = to_sc_camera_module(sd);
...
int sc_camera_module_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		if (SC_CAMERA_MODULE_POWER_OFF == cam_mod->state) {
		...
		if (SC_CAMERA_MODULE_HW_STANDBY == cam_mod->state) {
		...
		if (SC_CAMERA_MODULE_STREAMING == cam_mod->state) {
			ret = sc_camera_module_s_stream(sd, 0);
			if (!IS_ERR_VALUE(ret))
				cam_mod->state = SC_CAMERA_MODULE_SW_STANDBY;
		}
		if (SC_CAMERA_MODULE_SW_STANDBY == cam_mod->state) {
...
int sc_camera_module_s_ext_ctrls(
	struct v4l2_subdev *sd,
	struct v4l2_ext_controls *ctrls)
{
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		case V4L2_CID_FOCUS_ABSOLUTE:
			...
			ctrl_updt = SC_CAMERA_MODULE_CTRL_UPDT_FOCUS_ABSOLUTE;
	...
	if (ctrl_cnt &&
		...
		sc_ctrls.ctrls = kmalloc_array(ctrl_cnt,
			sizeof(struct sc_camera_module_ext_ctrl),
			GFP_KERNEL);
...
int sc_camera_module_get_flip_mirror(
	struct sc_camera_module *cam_mod)
{
	int mode = 0;
...
int sc_camera_module_enum_frameintervals(
    struct v4l2_subdev *sd,
...
int sc_camera_module_write_reglist(
	struct sc_camera_module *cam_mod,
...
int sc_camera_module_write_reg(
	struct sc_camera_module *cam_mod,
	u16 reg,
	u8 val)
{
...
int sc_camera_module_read_reg_table(
	struct sc_camera_module *cam_mod,
	u16 reg,
	u32 *val)
{
